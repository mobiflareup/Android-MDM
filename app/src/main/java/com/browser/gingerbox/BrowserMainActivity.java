...
import com.mobiocean.service.UpLoadDocsIntentService;
...
public class BrowserMainActivity extends Activity {
	...
	public class CustomChromeClient extends WebChromeClient {
		...
		public boolean onCreateWindow(WebView view, boolean isDialog,
			...
			browserHandler.postDelayed(new Runnable() {
				@Override
				public void run() {
					main[pageId].customLoadURL(getUrl.getText().toString());
					DeBug.ShowLog("Browser", "OnCreateWindow 181" + getUrl.getText().toString());
				}
			}, 500);
			return true;
		}
			...
			try {
				Thread downloader = new Thread(new Runnable() {
					...
					public void run() {
						...
						DeBug.ShowLog("BrowserMainActivity", "Downloading" + fileName);
						download.enqueue(it);
					}
				});
				downloader.run();
			} catch (NullPointerException e) {
				DeBug.ShowLog("BrowserMainActivity", "Problem downloading");
	...
	public class CustomWebViewClient extends WebViewClient {
		...
		{
			numberPage = view.getId();
			DeBug.ShowLog("Pageid","On Page Started --"+numberPage);
			if (view.isShown()) {
				...
				pageIsLoading = true;
				bOnPageStarted = true;
				DeBug.ShowLog("Pageid", "Onpage started" +url);
				boolean blockedFound = false;
				if((CallHelper.Ds.structCCC.wFeatureControlWord[0] & eFeatureControl.BROWSER_PROTECTION) == eFeatureControl.BROWSER_PROTECTION )
					if(bDataFound)
					{
						if((CallHelper.Ds.structCCC.wFeatureControlWord[0] & eFeatureControl.BROWSER_PROTECTION) == eFeatureControl.BROWSER_PROTECTION )
						{
							
//							blockedFound = checkBlockedInTime(url);
							blockedFound = blockUrl(url);
						}
						
						if(!blockedFound)
						if((CallHelper.Ds.structCCC.wFeatureControlWord[0] & eFeatureControl.BROWSER_BLOCK_PORN) == eFeatureControl.BROWSER_BLOCK_PORN )						
						{
							OnPageStartedRunner = new URLcheckTaskRunnerForOnPageStarted();

							if(AsyncTask.Status.RUNNING != OnPageStartedRunner.getStatus())
								OnPageStartedRunner.execute(url);
						}
						
						if((CallHelper.Ds.structCCC.wFeatureControlWord[0] & eFeatureControl.WEB_LOG) == eFeatureControl.WEB_LOG )						
						{
							checkKeWordInTime(url);
						}
						
					}
					else
					{
						DeBug.ShowLog("EN", "OnPage  "+bDataFound);
					}
	...
	public void generateHistory(final CustomWebView view) {
		...
		Thread history = new Thread(new Runnable() {
			...
			public void run() {
				...
				view.customLoadURL("file:///" + historyWebPage);
			}
					...
					DialogInterface.OnClickListener dialogClickListener = new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							case DialogInterface.BUTTON_NEGATIVE: {
								main[pageId].customLoadURL(result.getExtra());
								break;
							}
												...
												new Runnable() {
													...
													public void run() {
														...
														it.setDestinationInExternalPublicDir(
																...
																fileName);
														DeBug.ShowLog("BrowserMainActivity",
																"Downloading"
																		+ fileName);
														download.enqueue(it);
													}
	...
	public final Context CONTEXT = BrowserMainActivity.this;
	...
	protected static SharedPreferences.Editor sEdit;
	protected static WebPageViewHistoryStruct mWebPageViewHistoryStruct = null;
	protected static WebPageViewHistorySQLiteOpenHelper mWebPageViewHistorySQLiteOpenHelper = null;
	protected static ArrayList<WebPageViewHistoryStruct> urlHistoryList = new ArrayList<WebPageViewHistoryStruct>();

	// #GS

	protected static String stWebsiteCheckAPIWOTPart1 =  "http://api.mywot.com/0.4/public_link_json2?hosts="; 
	protected static String stEnteredURLbyUser;
	protected static String stWebsiteCheckAPIWOTPart2 = "/&callback=process&key=";
	protected static String stWebsiteCheckAPIWOT = "178eae0593698951c3920b8200bee5684b3c0dd6";
	protected static int[] WebContentRatingArray={101,401,402,403};

	protected static ArrayList<String> blockedWebsites;

	protected static boolean bAdultWebBlockONByParent = true;
	protected static ArrayList<String> stRestrictedWebsitesCache;
	protected static int CacheLimit = 5000;
	//	protected static boolean bCheckforAdultPage=false;

	protected static ArrayList<String> arr = new ArrayList<String>();
	URLcheckTaskRunnerForNewTab runner;// = new URLcheckTaskRunner();
	URLcheckTaskRunnerForOnPageStarted OnPageStartedRunner;
	CustomWebView view;// = new CustomWebView(this);
	boolean bAdultContent= false;
	boolean bNewTabOpen= false;
	boolean bOnPageStarted = false;
	boolean bDataFound=false;
	//boolean bLetParentWebsitesON =false;
		...
		Thread updateAutoComplete = new Thread(new Runnable() {
			...
			public void run() {
				...
				if (c != null) {
					...
					DeBug.ShowLog("BrowserMainActivity", "detected AOSP browser");
				} else {
					noStockBrowser = true;
					DeBug.ShowLog("BrowserMainActivity", "did not detect AOSP browser");
				}
				if (c != null) {
					c.close();
				}
		...
		getUrl.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				try {
					...
					main[pageId].customLoadURL(str);
					DeBug.ShowLog("Browser", "OnItemclick 1524" + str);
					setUrlText(str);
					getUrl.setPadding(tenPad, 0, tenPad, 0);
					InputMethodManager imm = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
					imm.hideSoftInputFromWindow(getUrl.getWindowToken(), 0);
				} catch (NullPointerException e) {
					DeBug.ShowLog("BrowserMainActivity Error: ",
...
//						Browser.clearHistory(getContentResolver());
			...
			DeBug.ShowLog("Lightning", "Error Clearing data");
		}
	...
	void init() {
		...
		// imageLarg dimensions and initialization
		final int dps = 175;
		...
		browserHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case 1: {
					main[pageId].customLoadURL(getUrl.getText().toString());
					DeBug.ShowLog("Browser", "InInt 1846+ "+getUrl.getText().toString());
					break;
				}
				case 2: {
			...
			for (String aMemoryURL : memoryURL) {
				if (aMemoryURL.length() > 0) {
					if (number == 0) {
						...
						main[number - 1].customLoadURL(aMemoryURL);
					} else {
	...
	public CustomWebView makeTab(final int pageToView, final String Url,final boolean display)
	{
		view = new CustomWebView(this);
		runner = new URLcheckTaskRunnerForNewTab();
		bNewTabOpen = true;
		...
		}  else 
		{
			if(bDataFound)
				...
				DeBug.ShowLog("EN", "Async  "+bDataFound);
			DeBug.ShowLog("Pageid","Make Tab --"+Url);
		}

		//	DeBug.ShowLog("Pageid","arr --"+arr+"Index --  "+arr.indexOf(Url));
		DeBug.ShowLog("BrowserMainActivity", "tab complete");
		return view;
	}

	private class URLcheckTaskRunnerForNewTab extends AsyncTask<String, String, String> 					// #GS
	{
		...
		protected String doInBackground(String... params)
		{
			...
			bAdultContent=false;
			String blocked =null;
			DeBug.ShowLog("EXP", "Async Task  --  "+URL);
			try {
				if(!blockedWebsites.isEmpty())
				{
					for(int i=0;i<blockedWebsites.size();i++)
					{
						blocked=blockedWebsites.get(i);
				...
				if(!stRestrictedWebsitesCache.isEmpty() && !bAdultContent)
				{
					for(int i=0;i<stRestrictedWebsitesCache.size();i++)
					{
						if(stEnteredURLbyUser.contains(stRestrictedWebsitesCache.get(i)))
						{
							bAdultContent=true;
							DeBug.ShowLog("EXP", "Async bAdultContent  --  "+bAdultContent);
							break;
						}
					}
		...
		protected void onPostExecute(String result) {
			...
			//  CheckSiteContentSafety(URL);
			DeBug.ShowLog("EXP", "Async bAdultContent onPostExecute  --  "+bAdultContent);
			if(bAdultContent)
			{
				view.customLoadURL("about:home");
				ShowPopUpWhenRestrictedWebsite();
			}

			else
			{
				view.customLoadURL(URL); 
			}
	...
	@SuppressWarnings("deprecation")
	int newTab(int theId, final String theUrl, final boolean display,final boolean incognito_mode)
	{
		DeBug.ShowLog("BrowserMainActivity", "making tab");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (first == 0) { // This dialog alerts the user to some navigation
			...
			builder.setTitle(Html.fromHtml("<p><b><u>BROWSER TIPS</u></b></p>"));
			builder.setMessage("WELCOME to the GingerBox Safe Browser."
					+"\n\nLong-press back button to exit browser"
					+"\n\nClick '2' Times on the back button to go to the Previous Page"
	...
	private class URLcheckTaskRunnerForOnPageStarted extends AsyncTask<String, String, String>			// #GS 
	{
		...
		protected void onPostExecute(String result) {
			...
			if(sf)
			{
				DeBug.ShowLog("Pageid","sf  "+sf);
				main[pageId].customLoadURL("www.google.co.in");
			}else if(!bOnPageStarted)
				main[pageId].customLoadURL("www.google.co.in");
	...
	private boolean checkPornInTime(String URL)
	{
		boolean sf = false;
		...
		DeBug.ShowLog("features",  " WF bPresentlyStopped" + CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]);
		DeBug.ShowLogD("features", "WF lAllowedTime"+Long.toHexString(CallHelper.Ds.structFCC.lAllowedTime[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN][Hr]));

		if(CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN])
		{	  
		//	sf = checkPorn(URL);
		}	  
		else
		{	 
			DeBug.ShowLog("features", "Day "+CallHelper.Ds.structPC.bWeekDay+" AppHrMin "+ CallHelper.Ds.structPC.AppHrMin);

			if(CallHelper.Ds.structFCC.bTimeDurationCtrl[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]==0) 
			{
				CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN] = false;
				CallHelper.Ds.structFCC.bPresentlyOn[eFeatureControl.iBROWSER_BLOCK_PORN] = 3;
			}
			else
			{
				isCurrentlyBlockedInTime = false;
				DeBug.ShowLogD("features", "WF lAllowedTime"+Long.toHexString(CallHelper.Ds.structFCC.lAllowedTime[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN][Hr]));
				if((CallHelper.Ds.structFCC.bTimeDurationCtrl[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN] & eFeatureSetting.RESTRICTED_IN_CLOCK_TIME) == eFeatureSetting.RESTRICTED_IN_CLOCK_TIME)
				{

					if((CallHelper.Ds.structFCC.lAllowedTime[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN][Hr] & MIN_LUT[Min]) != 0)
					{
						isCurrentlyBlockedInTime = true;
						sf = checkPorn(URL);		
					}
					DeBug.ShowLog(Tag1,""+ "   HR "+Hr +" Min "+Min);

				}

				if(!isCurrentlyBlockedInTime)
					if((CallHelper.Ds.structFCC.bTimeDurationCtrl[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN] & eFeatureSetting.RESTRICTED_IN_DURATION) == eFeatureSetting.RESTRICTED_IN_DURATION)
					{
						if(! CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN])		            	 
						{
							CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]++;
							CallHelper.Ds.structFCC.bPresentlyOn[eFeatureControl.iBROWSER_BLOCK_PORN] = 1;
							DeBug.ShowLog("features",  "WF RESTRICTED_IN_DURATION "+CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]+" "+  CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]);

							sf = checkPorn(URL);	
						}
						if(CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]/60 >= CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN])
						{	 
							...
							CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN] = true;
							CallHelper.Ds.structFCC.bPresentlyOn[eFeatureControl.iBROWSER_BLOCK_PORN] = 3;
							DeBug.ShowLog("features",  "WF RESTRICTED_IN_DURATION . Used  Total.   "+CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]/60 +" "+  CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]);
							//DeBug.ShowToast(AppBlock.this, "WF RESTRICTED_IN_DURATION . WF . \n"+CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]/60 +" "+  CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_PORN]);
						}
					}
			}
		}
	...
	private boolean checkPorn(String URL)
	{
		...
		try {
			...
			if(!stRestrictedWebsitesCache.isEmpty())
			{
				for(int i=0;i<stRestrictedWebsitesCache.size();i++)
				{
					if(URL.contains(stRestrictedWebsitesCache.get(i)))
					{
						sf = true;
						break;
					}
			...
			if(!sf)
			{
				DeBug.ShowLog("Pageid","sf CheckSiteContentSafety "+sf);
				sf = CheckSiteContentSafety(URL);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	private boolean checkBlockedInTime(String URL)
	{
		boolean sf = false;
		...
		boolean isCurrentlyBlockedInTime = false; 
		DeBug.ShowLog("features",  " WF bPresentlyStopped" + CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]);
		DeBug.ShowLogD("features", "WF lAllowedTime"+Long.toHexString(CallHelper.Ds.structFCC.lAllowedTime[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION][Hr]));

		if(CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION])
		{	  
			sf = checkBlocked(URL);							
		}	  
		else
		{	 
			...
			if(CallHelper.Ds.structFCC.bTimeDurationCtrl[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]==0) 
			{
				CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION] = false;
				CallHelper.Ds.structFCC.bPresentlyOn[eFeatureControl.iBROWSER_PROTECTION] = 3;
			}
			else
			{
				isCurrentlyBlockedInTime = false;
				DeBug.ShowLogD("features", "WF lAllowedTime"+Long.toHexString(CallHelper.Ds.structFCC.lAllowedTime[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION][Hr]));
				if((CallHelper.Ds.structFCC.bTimeDurationCtrl[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION] & eFeatureSetting.RESTRICTED_IN_CLOCK_TIME) == eFeatureSetting.RESTRICTED_IN_CLOCK_TIME)
				{

					if((CallHelper.Ds.structFCC.lAllowedTime[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION][Hr] & MIN_LUT[Min]) != 0)
					{
						isCurrentlyBlockedInTime = true;
						sf = checkBlocked(URL);		
					}
				...
				if(!isCurrentlyBlockedInTime)
					if((CallHelper.Ds.structFCC.bTimeDurationCtrl[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION] & eFeatureSetting.RESTRICTED_IN_DURATION) == eFeatureSetting.RESTRICTED_IN_DURATION)
					{
						if(! CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION])		            	 
						{
							CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]++;
							CallHelper.Ds.structFCC.bPresentlyOn[eFeatureControl.iBROWSER_PROTECTION] = 1;
							DeBug.ShowLog("features",  "WF RESTRICTED_IN_DURATION "+CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]+" "+  CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]);

							
						}
						if(CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]/60 >= CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION])
						{	 
							// check if the feature is running with a Flag, then check if the duration is over, if not then let this activity to start/continue and
							//1. when the activity is being stopped then it saves the current duration,
							//2. if the duration has expired then stops the activity and save the current duration.
							CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION] = true;
							CallHelper.Ds.structFCC.bPresentlyOn[eFeatureControl.iBROWSER_PROTECTION] = 3;
							DeBug.ShowLog("features",  "WF RESTRICTED_IN_DURATION . Used  Total.   "+CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]/60 +" "+  CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]);
							//DeBug.ShowToast(AppBlock.this, "WF RESTRICTED_IN_DURATION . WF . \n"+CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]/60 +" "+  CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_PROTECTION]);
						}
					}
			}
		}
	...
	private boolean checkBlocked(String URL)
	{
		...
		if(!blockedWebsites.isEmpty())
		{
			for(int i=0;i<blockedWebsites.size();i++)
			{
				String blocked=blockedWebsites.get(i).toLowerCase().trim();
				if(URL.contains(blocked))
				{
	...
	private boolean checkKeWordInTime(String URL)
	{
		...
		boolean isCurrentlyBlockedInTime = false; 
		DeBug.ShowLog("features",  " WF bPresentlyStopped" + CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG]);
		DeBug.ShowLogD("features", "WF lAllowedTime"+Long.toHexString(CallHelper.Ds.structFCC.lAllowedTime[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG][Hr]));

		if(CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG])
		{
			if((CallHelper.Ds.structFCC.lAllowedTime[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG][Hr] & MIN_LUT[Min]) != 0)
				sf = sendKeyword(URL);
		}	  
		else
		{	 
			...
			if(CallHelper.Ds.structFCC.bTimeDurationCtrl[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG]==0) 
			{
				CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG] = false;
				CallHelper.Ds.structFCC.bPresentlyOn[eFeatureControl.iWEB_LOG] = 3;
			}
			else
			{
				isCurrentlyBlockedInTime = false;
				DeBug.ShowLogD("features", "WF lAllowedTime"+Long.toHexString(CallHelper.Ds.structFCC.lAllowedTime[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG][Hr]));
				if((CallHelper.Ds.structFCC.bTimeDurationCtrl[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG] & eFeatureSetting.RESTRICTED_IN_CLOCK_TIME) == eFeatureSetting.RESTRICTED_IN_CLOCK_TIME)
				{
				...
				if(!isCurrentlyBlockedInTime)
					if((CallHelper.Ds.structFCC.bTimeDurationCtrl[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG] & eFeatureSetting.RESTRICTED_IN_DURATION) == eFeatureSetting.RESTRICTED_IN_DURATION)
					{
						if(! CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG])		            	 
						{
							CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG]++;
							CallHelper.Ds.structFCC.bPresentlyOn[eFeatureControl.iWEB_LOG] = 1;
							DeBug.ShowLog("features",  "WF RESTRICTED_IN_DURATION "+CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG]+" "+  CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG]);
							
							//sf = sendKeyword(URL);	
						}
						if(CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG]/60 >= CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG])
						{	 
							// check if the feature is running with a Flag, then check if the duration is over, if not then let this activity to start/continue and
							//1. when the activity is being stopped then it saves the current duration,
							//2. if the duration has expired then stops the activity and save the current duration.
							CallHelper.Ds.structFCC.bPresentlyStopped[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG] = true;
							CallHelper.Ds.structFCC.bPresentlyOn[eFeatureControl.iWEB_LOG] = 3;
							DeBug.ShowLog("features",  "WF RESTRICTED_IN_DURATION . Used  Total.   "+CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG]/60 +" "+  CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iWEB_LOG]);
							//DeBug.ShowToast(AppBlock.this, "WF RESTRICTED_IN_DURATION . WF . \n"+CallHelper.Ds.structFCC.wUsedDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_KEYWORD]/60 +" "+  CallHelper.Ds.structFCC.wTotalDuration[CallHelper.Ds.structPC.bWeekDay][eFeatureControl.iBROWSER_BLOCK_KEYWORD]);
						}
					}
			}
		}
...
//		SharedPreferences settings_1;
//		settings_1 = getSharedPreferences(CallHelper.PREFS_NAME, 0);
//		CallHelper.decodeMessage(this, settings_1.getString("webCode", ""));
		boolean sf = true;
//		int Hr = CallHelper.Ds.structPC.AppHrMin/100;
//		int Min = CallHelper.Ds.structPC.AppHrMin%100;
//		WebsiteDB websiteDB = WebsiteDB.getInstance(BrowserMainActivity.this);
//		{
...
//						if(blockTime!=null) {
//							if(blockTime[CallHelper.Ds.structPC.bWeekDay-1][totMin]){
//								DeBug.ShowLog("NarayananWB","WEBBLOCK CHECK");
//								if(sf)
//									sf = websiteDB.isUrlAllowed(URL, cat);
//							}else{
...
////			}
//		}
		sf = CallHelper.blockUrl(URL, BrowserMainActivity.this);
			...
			handler.post(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						jsonMemoryInfo.put("WebsiteUrl", tempUrl);
	...
	private boolean sendKeyword(String URL)
	{
		boolean sf = false;

		
		stEnteredURLbyUser=getUrl.getText().toString();
		URL =stEnteredURLbyUser;
		...
		mWebPageViewHistoryStruct = new WebPageViewHistoryStruct();
		mWebPageViewHistorySQLiteOpenHelper = WebPageViewHistorySQLiteOpenHelper.getInstance(CONTEXT);

		int startindex  = URL.indexOf("://");
		int index  = URL.indexOf("/", startindex+3);
		
		if(index==-1)
			index= URL.length();
		URL = URL.substring(0,index);
		mWebPageViewHistoryStruct.setPagename(URL);
		mWebPageViewHistoryStruct.setLogdatetime(SMSTimeStamp);

		urlHistoryList.add(mWebPageViewHistoryStruct);

		for(int i=0; i<urlHistoryList.size();i++)
		{
			mWebPageViewHistoryStruct.set_uploadation_Status(1);
			mWebPageViewHistoryStruct.setPagename(urlHistoryList.get(i).getPagename());
			mWebPageViewHistoryStruct.setLogdatetime(urlHistoryList.get(i).getLogdatetime());
			DeBug.ShowLog("Browser", "Enter Url 2335: " + mWebPageViewHistoryStruct.getPagename());
			mWebPageViewHistorySQLiteOpenHelper.addContact(mWebPageViewHistoryStruct);
		}

		DeBug.ShowLog("Browser", "Enter Url 2335: " + urlHistoryList.size());
		urlHistoryList.clear();

		Intent startDataservice2 =  new Intent(getApplicationContext(),UpLoadDocsIntentService.class);
		Bundle sendData2=new Bundle();
		sendData2.putInt("FeatureIndex", (int)1);
		sendData2.putInt("FeatereValue", (int)6);
		startDataservice2.putExtras(sendData2);
		getApplicationContext().startService(startDataservice2);
	
		return sf;
	}
	private void ShowPopUpWhenRestrictedWebsite()						// #GS
	{
		DialogInterface.OnClickListener dialogClickListener = new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(CONTEXT); // dialog
		builder.setTitle(Html.fromHtml("<p><b>Restricted Website Access</b></p>"));
		builder.setMessage("You are not allowed to Visit this Website")
		.setPositiveButton("Ok", dialogClickListener).show();
	}

	private boolean CheckSiteContentSafety(String stEnteredURLbyUser)						// #GS
	{
		boolean isPorn = false;
		int count=0;
		String stProcessedString = null;
		try {
			int p =stEnteredURLbyUser.indexOf("/", 0);
			int q = stEnteredURLbyUser.indexOf("/", p+2);
			if(p>=0)
			{
				stProcessedString= stEnteredURLbyUser.substring(p+2, q);
			}
			//	DeBug.ShowLog("EXP", stEnteredURLbyUser);
			//	DeBug.ShowLog("EXP", "Data -- -" +p +q);
			DeBug.ShowLog("EXP", "String -- -" +stProcessedString);
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		stEnteredURLbyUser = stProcessedString;
		String stKey = null;
		int[] WebResponseAfterQuery = new int [10];


		StringBuilder uri = new StringBuilder(stWebsiteCheckAPIWOTPart1+stEnteredURLbyUser+stWebsiteCheckAPIWOTPart2+stWebsiteCheckAPIWOT);
		...
		try{
			...
			BufferedReader bfrd = new BufferedReader(new InputStreamReader(in),1);
			for (String line = null; (line = bfrd.readLine()) != null;) 
			{
				responce=responce+line;
			}

			responce =responce.replace("process", "");
			...
			try {
				...
				JSONObject json1 = json.getJSONObject(stEnteredURLbyUser);
				JSONObject jsonObj = json1.getJSONObject("categories");
				Iterator<?> keys = jsonObj.keys();

				while( keys.hasNext() )
				{
					stKey = (String)keys.next();
					WebResponseAfterQuery[count] = Integer.parseInt(stKey);
					//	DeBug.ShowLog("EXP", "Data  ----   "+ WebResponseAfterQuery[count]);
					count++;
				}


			} catch (JSONException e) {
				//	DeBug.ShowLog("EXP", "Check Website Adult Exception");
				e.printStackTrace();
			}
		...
		for(int i=0;i<WebContentRatingArray.length;i++)
		{
			for(int j= 0 ;j<count;j++)
				if(WebResponseAfterQuery[j] ==WebContentRatingArray[i])
				{
					//	DeBug.ShowLog("EXP", ""+responce+ "Yes");
					if(bNewTabOpen)
						bAdultContent=true;
					if(bOnPageStarted)
						bOnPageStarted = false;
					isPorn =true;
					stRestrictedWebsitesCache.add(stEnteredURLbyUser);
					break;
				}

		}

		sEdit.putInt("stRestrictedWebsitesCacheSize", stRestrictedWebsitesCache.size());
		for(int i=0;i<stRestrictedWebsitesCache.size();i++)
			sEdit.putString("stAdultWebsite"+i, stRestrictedWebsitesCache.get(i));
		sEdit.commit();
	...
	protected void onResume() {
		super.onResume();
		blockedWebsites = new ArrayList<String>();
		blockedWebsites.clear();
		stRestrictedWebsitesCache = new ArrayList<String>();
		try {
			stRestrictedWebsitesCache.clear();
			stRestrictedWebsitesCache.add("www.goldtube8.com");
			stRestrictedWebsitesCache.add("www.nakedtube8.com");
			stRestrictedWebsitesCache.add("www.pimptubed.com");
		} catch (Exception e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		if(stRestrictedWebsitesCache.size()>CacheLimit)
		{
			stRestrictedWebsitesCache.clear();
		}

		//Shared Preference for Adult Wesite Cache
		sPref=PreferenceManager.getDefaultSharedPreferences(getApplicationContext());
		sEdit=sPref.edit();
		int size=sPref.getInt("stRestrictedWebsitesCacheSize",0);
		for(int j=0;j<size;j++)
		{
			stRestrictedWebsitesCache.add(sPref.getString("stAdultWebsite"+j,null));
		}


		/*try 
		{
			con = createPackageContext("com.gingerboxsg", 0);
			bDataFound=true;			
		} catch (NameNotFoundException e) 
		{
			DeBug.ShowLog("Websites", "No Data Found in gingerboxsg");
		}

		try 
		{
			con = createPackageContext("com.stuaid", 0);			
			bDataFound=true;			
		} catch (NameNotFoundException e) 
		{
			DeBug.ShowLog("Websites", "No Data Found in gingerbox");
		}*/
		bDataFound = true;
		if(bDataFound)
		{
			try 
			{
				...
				SharedPreferences pref = getSharedPreferences("blockedWebsitesPref", Context.MODE_MULTI_PROCESS);
				bAdultWebBlockONByParent = pref.getBoolean("structFCC.bAdultWebBlockONByParent", false);
				int data = pref.getInt("stBlockedWebsitesName_Size", 0);
				for(int i=0;i<data;i++)
				{
					String WebSite = pref.getString(""+i, "");
					DeBug.ShowLog("BlockWeb",""+WebSite);
					String decyptedWebsites=StringCryptor.decrypt(WebSite, StringCryptor.encryptionKey);
					DeBug.ShowLog("BlockWeb", ""+decyptedWebsites);
					blockedWebsites.add(decyptedWebsites);
				}
				DeBug.ShowLog("Websites", ""+blockedWebsites);
	...
	void openBookmarks(CustomWebView view) {
		...
		view.customLoadURL("file:///" + bookmarkWebPage);
			...
			update = new Runnable() {
				...
				public void run() {
					try {
						...
						DeBug.ShowLog("BrowserMainActivity", "ERRRRROOORRRR 1");
					} catch (NullPointerException e) {
						DeBug.ShowLog("BrowserMainActivity", "ERRRRROOORRRR 2");
					} catch (SQLiteException e) {
						DeBug.ShowLog("BrowserMainActivity", "SQLiteException");
					}
				}
			};
			try {
